...
M = '\x1b[1;37m'#ابیض
#from uuid import uuid4
proxy = ['d7afd2e4464b27e85df924eda0fe4bcc','00c80e31dcdf1af82ed939608590d1f2']
#uid = uuid4().hex.upper()
#print(uid)
...
tok=input(X+'➤ ͲΌᏦᎬΝ  : '+F)
os.system('clear')
session_id = random.choice(proxy)
print(session_id)
os.system('clear')
def info(email):
  global hit,iid,tok
...
def req(email):
    ...
    if ("1023")in re:
         Gmail(email)
         os.system('cls'if os.name=='nt'else'clear')
        # print('      انتظر جارً تشغيل الاداة ')
         gt+=1
         print(f"\r  o   {F}True {HH}: {M}{ge} ~ {Z}False {HH}: {M}{bt} ~ {X}Not {HH}: {M}{be} {G}\r")
    else:
        os.system('cls'if os.name=='nt'else'clear')
        bt+=1
        print(f"\r{F} • Hit {HH}: {M}{ge} \n {Z}• incorrect {HH}: {M}{bt} \n {X}• Bad {HH}: {M}{be} {G} \n------------------------- \n • Dev : @F_M_D \r")
def hso1():
        while True:
            try:
                g=choice(
